...
function initCountriesAndStates() {
	...
	$.ajax({
		...
		success: function (msg) {
			...
			//initially set to DE
			$("#contact_state_control_select").val("DE").trigger("change");
...
function refreshServices() {
	...
	//ra package hack
	var $ra = $("#ra_control");
	var inverse_pid = $ra.data("inversepackageproductid");
	var alternate_pid = $($.grep($orderpackage.find("option"), function (item, i) { return $(item).val() != inverse_pid; })[0]).val();
	var valid_selected = $orderpackage.val() != inverse_pid;
	if ($ra.prop("checked") != valid_selected) $ra.prop("checked", valid_selected);
...
function initType() {
	...
	var $companytypes = $(":radio[name='company_type']");
	var $companystate = $("#company_state_control");
	var $companytype = $("#company_type_control");
	...
	$companytypes.on("click change", function () {
		var t = $companytypes.filter(":checked").val();
...
function initContact() {
	...
	$("#contact_state_control_select").on("change", function () { _isdelaware = $(this).val() == "DE" && !_isinternational; refreshServices(); });
...
function initServices() {
	...
	//ra package hack
	var $ra = $("#ra_control");
	$ra.on("click change", function () {
		var $orderpackage = $("#order_package_control");
		var inverse_pid = $ra.data("inversepackageproductid");
		var selected = $ra.prop("checked");
		var alternate_pid = $($.grep($orderpackage.find("option"), function (item, i) { return $(item).val() != inverse_pid; })[0]).val();

		var valid_pid = selected ? alternate_pid : inverse_pid;
		if (valid_pid != $orderpackage.val()) $orderpackage.val(valid_pid).trigger("change");
	});

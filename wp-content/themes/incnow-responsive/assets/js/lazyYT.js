...
;(function ($) {
    ...
    function setUp($el, settings) {
        ...
        if (display_title) {
			...
			// Trying to create some HTML to put after the wrapper div of js-lazyYT         
			$el.after('<p id="lazyYT-title-' + id + '" class="html5-title-text">Loading...</p>');
        }
        ...
        if ((!title && display_title) || display_duration) {
            ...
            $.getJSON(youtube_data_url.join(''), function (data) {
				...
				$el.closest('.four').find('#lazyYT-title-' + id).text(item.snippet.title);
